...
# The Flask app is in /fyp/web-test, so thesis files are in ../uploads/approved/
BASE_UPLOAD_PATH = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'uploads', 'approved'))
            ...
            for doc_idx, match in enumerate(similar_files_list, 1):
                ...
                if similar_sentences and len(similar_sentences) > 0:
                    ...
                    for sent_idx, sentence in enumerate(similar_sentences, 1):
                        ...
                        # Your Document - paragraph format
                        your_doc_label = ParagraphStyle(
                            f'YourDocLabel{doc_idx}{sent_idx}',
                            parent=styles['Normal'],
                            fontSize=9,
                            textColor=colors.HexColor('#34568B'),
                            fontName='Helvetica-Bold',
                        ...
                        elements.append(Paragraph("<b>Your Document:</b>", your_doc_label))
                        
                        your_doc_text = ParagraphStyle(
                            f'YourDocText{doc_idx}{sent_idx}',
                            parent=styles['Normal'],
                            fontSize=9,
                        ...
                        uploaded_text = sentence.get('uploaded', 'N/A')
                        elements.append(Paragraph(uploaded_text, your_doc_text))
                        
                        # Existing Document - paragraph format
                        existing_doc_label = ParagraphStyle(
                            f'ExistingDocLabel{doc_idx}{sent_idx}',
                            parent=styles['Normal'],
                            fontSize=9,
                            textColor=colors.HexColor('#34568B'),
                            fontName='Helvetica-Bold',
                        ...
                        elements.append(Paragraph("<b>Existing Document:</b>", existing_doc_label))
                        
                        existing_doc_text = ParagraphStyle(
                            f'ExistingDocText{doc_idx}{sent_idx}',
                            parent=styles['Normal'],
                            fontSize=9,
                            textColor=colors.black,
                            spaceAfter=10,
                        ...
                        existing_text = sentence.get('existing', 'N/A')
                        elements.append(Paragraph(existing_text, existing_doc_text))
